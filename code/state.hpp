...
double reconstruct_marginal_ancestral_state_decomp(const evo_tree& rtree, map<int, vector<vector<int>>>& vobs, const set<vector<int>>& comps, OBS_DECOMP& obs_decomp, int use_repeat, int infer_wgd, int infer_chr, int cn_max, ofstream& fout, double min_asr, int is_total = 1);
...
void reconstruct_joint_ancestral_state_decomp(const evo_tree& rtree, map<int, vector<vector<int>>>& vobs, const set<vector<int>>& comps, MAX_DECOMP& max_decomp, int use_repeat, int cn_max, ofstream& fout, double min_asr, int is_total = 1);
