...
code/sveta -o $dir -r $Ns -n $Nsim --cn_max $cn_max --dup_rate $r1 --del_rate $r2 --chr_gain $r3 --chr_loss $r4 --wgd $r5 --dup_size $s1 --del_size $s2 -e $Ne -t $dt --verbose $verbose --constrained $cons --model $model -p "$prefix" --age $age > $dir/std_test_sveta_cons"$cons"_model"$model"
# --seed $seed
