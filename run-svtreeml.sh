...
if [[ $mode -eq 1 ]]; then
  ...
  if [[ $bootstap -eq 1 ]]; then
    ...
    do
      ...
      code/svtreeml -c $input -t $times -s $Ns -p $Npop -g $Ngen -e $Nstop -o $ofile -d $model --cn_max $cn_max --only_seg $only_seg --tree_search $tree_search --init_tree $init_tree --dir_itrees $dir_itrees --optim $opt --constrained $cons --fixm $maxj --correct_bias $correct_bias -x $mu --dup_rate $r1 --del_rate $r2  --chr_gain_rate $r3 --chr_loss_rate $r4 --wgd_rate $r5  --verbose $verbose -b 1 > $bdir/std-maxL-"$suffix"-btree-$i
    done
    ...
    Rscript ana/plot-trees-all.R -s $bdir -f $mltree -o $dir/results-maxL-tree-"$suffix"-bootstrap.pdf -t "bootstrap" -l "age" --time_file $times -p "results-maxL-"$suffix"-btree-*txt"
  fi
  ...
  Rscript ana/plot-trees-all.R -d test/ -b 0 -t "all" -l "age" --time_file $times -p "sim-data-*-tree.txt"
